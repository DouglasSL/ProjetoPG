  ...
  var paths = [
    new Path().stroke(PATH_COLOR, PATH_STROKE).addTo(stage),
    new Path().stroke(PATH_COLOR, PATH_STROKE).addTo(stage),
    new Path().stroke(PATH_COLOR, PATH_STROKE).addTo(stage),
    new Path().stroke(PATH_COLOR, PATH_STROKE).addTo(stage)
  ];
  ...
  var bezier_curves = [
    new Path().stroke(BEZIER_COLOR, BEZIER_STROKE).addTo(stage),
    new Path().stroke(BEZIER_COLOR, BEZIER_STROKE).addTo(stage),
    new Path().stroke(BEZIER_COLOR, BEZIER_STROKE).addTo(stage),
    new Path().stroke(BEZIER_COLOR, BEZIER_STROKE).addTo(stage)
  ];
  ...
  stage.on('click', function(clickEvent) {
    ...
    // stage.sendMessage('ready', {});
      ...
      if(target.id <= 2 && 'id' in target && countPoints < 16){
          ...
          if(all_points[0].length < 4){
              all_points[0].push(point.id);
          } else if(all_points[1].length < 4){
              all_points[1].push(point.id);
          } else if(all_points[2].length < 4){
              all_points[2].push(point.id);
          } else if(all_points[3].length < 4){
              all_points[3].push(point.id);
          }
  ...
  function draw(){
      ...
      for (t = 0; t <= 1; t += 1/sb){
          ...
          for (i = 0; i < evaluations; i++){
              controlPoints[0] = bezier_curves[0]._segments[i];

              controlPoints[1] = bezier_curves[1]._segments[i];

              controlPoints[2] = bezier_curves[2]._segments[i];

              controlPoints[3] = bezier_curves[3]._segments[i];
